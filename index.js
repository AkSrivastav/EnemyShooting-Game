...
const amit = new Player(playerPosition.x, playerPosition.y, 15, "white");
const weapons = [];
const enemies = [];
...
const spawnEnemy = () => {
  ...
  if (Math.random() < 0.5) {
    // making enemy's initial location
...
function animation() {
  ...
  amit.draw(); // drawing player

  weapons.forEach(
    (
      weapon,
      weaponIndex // generating bullets
    ) => {
      weapon.update();
  ...
  enemies.forEach(
    (
      ...
      if (distanceBetweenPlayerAndEnemy - amit.radius - enemy.radius < 1) {
...
canvas.addEventListener(
  "click",
  (
    e // Event Listener for Weapon (on left click)
  ) => {
    shootingSound.play();
    ...
    weapons.push(
      new Weapon(
        canvas.width / 2, // adding weapon in weapons array
        canvas.height / 2,
        6,
        "white",
        velocity,
